...
public class AlertScheduledTask {
    ...
    /**
     * 每天凌晨2:30生成"每日全量快照"（方案A）
     *
     * <p>与 2:00 的预警生成错开：无论学生当天是否触发预警（GREEN 也写），
     * 每个选了课的学生、每门课都写一条当天快照，保证风险趋势曲线连续。
     * 一人一课一天只保留一条，重复执行会先清理当天旧记录。
     */
    @Scheduled(cron = "0 30 2 * * ?")
    public void generateDailySnapshots() {
        log.info("【定时任务】开始生成每日全量快照...");
        long startTime = System.currentTimeMillis();
        try {
            int count = warningGenerationService.generateDailySnapshots();
            log.info("【定时任务】每日全量快照完成，共写入 {} 条，耗时 {} ms",
                    count, System.currentTimeMillis() - startTime);
        } catch (Exception e) {
            log.error("【定时任务】每日全量快照生成失败: {}", e.getMessage(), e);
        }
    }
     ...
     * 每天凌晨1点检查超时预警。
     *
     * <p>幂等保护：定时任务每天都会扫到同一批超时预警，这里按"预警ID + 通知类型 + 标题"
     * 判断是否已经发过，避免同一件事每天重复催办、重复提醒（此前会天天骚扰教师/学生）。
     */
    ...
    public void escalateTimeouts() {
        ...
        try {
            ...
            // 1. ACTIVE 超过 days1 天（且尚未超过 days2 天）→ 催办教师
            List<AlertRecord> active2d = alertRecordMapper.selectList(
                    new LambdaQueryWrapper<AlertRecord>()
                            ...
                            .ge(days2 > days1, AlertRecord::getCreateTime, now.minusDays(days2)));
            for (AlertRecord a : active2d) {
                ...
                String title = "⏰ 催办：" + name + "的预警已超" + days1 + "天未处理";
                if (alreadyNotified(a.getId(), "TIMEOUT", title)) { skipped++; continue; }
                notificationService.create(teacherId, "TEACHER", title,
                        name + "的" + levelText(a.getAlertLevel()) + "预警生成于" + fmt(a.getCreateTime())
                                + "，已超过" + days1 + "天未处理，请尽快处理。",
                        a.getId(), "TIMEOUT");
            }

            // 2. ACTIVE 超过 days2 天 → 严重提醒教师
            ...
            for (AlertRecord a : active5d) {
                ...
                Student s = studentMapper.selectById(a.getStudentId());
                String name = s != null ? s.getStudentName() : "学生" + a.getStudentId();
                String title = "🚨 严重提醒：" + name + "的预警已超" + days2 + "天未处理";
                if (alreadyNotified(a.getId(), "TIMEOUT", title)) { skipped++; continue; }
                notificationService.create(teacherId, "TEACHER", title,
            ...
            // 3. ACKNOWLEDGED 超过 daysAcked 天 → 催办教师
            ...
            for (AlertRecord a : acked3d) {
                ...
                Student s = studentMapper.selectById(a.getStudentId());
                String name = s != null ? s.getStudentName() : "学生" + a.getStudentId();
                String title = "⏰ 催办：" + name + "已确认预警，请及时处理";
                if (alreadyNotified(a.getId(), "TIMEOUT", title)) { skipped++; continue; }
                notificationService.create(teacherId, "TEACHER", title,
            ...
            // 4. HANDLED 超过 daysHandled 天 → 催学生回应
            ...
            for (AlertRecord a : handled3d) {
                ...
                String title = "⏰ 老师已处理你的预警，请回应";
                if (alreadyNotified(a.getId(), "TIMEOUT", title)) { skipped++; continue; }
                notificationService.create(a.getStudentId(), "STUDENT", title,
                        "老师已在" + daysHandled + "天前处理了你的学业预警，请登录系统查看处理意见并回应。",
                        a.getId(), "TIMEOUT");
            }

            // 5. HANDLED 超过 daysUpgrade 天 + 学生未回应 → 升级等级 + 通知双方
            ...
            for (AlertRecord a : handled5d) {
                // 已回应过 → 跳过
                if (a.getTeacherNote() != null && a.getTeacherNote().contains("【学生回应】")) continue;
                // 先记住原等级：setAlertLevel 之后 a.getAlertLevel() 就变成新等级了
                String oldLevel = a.getAlertLevel();
                String newLevel = levelUp(oldLevel);
                if (newLevel == null || newLevel.equals(oldLevel)) continue;
                ...
                // 通知学生
                String studentTitle = "⚠️ 预警升级：因未回应，风险等级已提升为" + levelText(newLevel);
                if (!alreadyNotified(a.getId(), "ALERT_UPGRADE", studentTitle)) {
                    notificationService.create(a.getStudentId(), "STUDENT", studentTitle,
                            "由于你超过" + daysUpgrade + "天未回应老师的处理意见，你的学业预警等级已从"
                                    + levelText(oldLevel) + "自动升级为" + levelText(newLevel) + "。请尽快登录查看并回应。",
                            a.getId(), "ALERT_UPGRADE");
                }

                // 通知教师
                if (teacherId != null) {
                    String teacherTitle = "⚠️ " + name + "的预警因未回应自动升级为" + levelText(newLevel);
                    if (!alreadyNotified(a.getId(), "ALERT_UPGRADE", teacherTitle)) {
                        notificationService.create(teacherId, "TEACHER", teacherTitle,
                                name + "超过" + daysUpgrade + "天未回应处理意见，预警已自动从"
                                        + levelText(oldLevel) + "升级为" + levelText(newLevel) + "。",
                                a.getId(), "ALERT_UPGRADE");
                    }
                }
            }

            log.info("【超时升级】完成，扫描 {} 条，升级 {} 条，跳过已催办 {} 条",
    ...
    /**
     * 该预警是否已经发过"同类型 + 同标题"的通知。
     * 定时任务每天都会扫到同一批超时预警，用它防止重复催办。
     */
    private boolean alreadyNotified(Long alertId, String type, String title) {
        if (alertId == null) return false;
        Long count = notificationMapper.selectCount(new LambdaQueryWrapper<Notification>()
                .eq(Notification::getAlertId, alertId)
